...
var instantclick
  ...
  function triggerPageEvent(eventType, ...args) {
  ...
  function addTimer(args, isRepeating, realDelay=0) {
    var callback = args[0]
      , delay = args[1]
  ...
  function popstateListener() {
    ...
    if (!(loc in $history)) {
      triggerPageEvent('exit', location.href)
  ...
  function display(url) {
    ...
    $history[$currentLocationWithoutHash].scrollPosition = pageYOffset
    setPreloadingAsHalted()
    changePage($title, $body, $url, window.location.pathname == (new URL($url)).pathname ? pageYOffset : 0)
  }
